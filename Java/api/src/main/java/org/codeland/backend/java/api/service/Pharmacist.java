...
public class Pharmacist implements UserServiceInterface{
	private ArrayList<User> users = UserDatabase.getUsers();
	public User login (User userToLogin) {
		if(userToLogin.getRole().equals("admin") && userToLogin.getPassword().length() == 5) {
			Integer count = 0;
			User user = new User();
			for(User u : users){
		...
		userToLogin.setResponseMsg("Password must be 5 digits");
		userToLogin.setUserFound(false);
		return userToLogin;
	}
	public User signupUser(User user) {
		if(user.getRole().equals("admin") && user.getPassword().length() == 5) {
			user.setId(users.size() + 1);
			users.add(user);
			return user;
		}
		user.setResponseMsg("Password must be 5 digits");
		return user;
	}
}
